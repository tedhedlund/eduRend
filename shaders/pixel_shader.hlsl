...
float4 PS_main(PSIn input) : SV_Target
{
	...
	float4 cubeTexture = texCube.Sample(cubeSampler, viewVector);
	float4 sceneColor = (Ka * color) + (diffuse * color) + (specular);
	float4 finalColor = lerp(cubeTexture, sceneColor, color.a);
	return finalColor;
